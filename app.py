...
# def apply_area_penalty(area, yield_pred):
#     # Define the area thresholds
#     lower_threshold = 0.5
#     upper_threshold = 2.0
#
#     # Calculate the penalty using a softer penalty formula
#     if area < lower_threshold:
#         area_penalty = 1 - ((lower_threshold - area) / lower_threshold) * 0.2
#     elif area > upper_threshold:
#         area_penalty = 1 - ((area - upper_threshold) / upper_threshold) * 0.2
#     else:
#         area_penalty = 1.0
#
#     # Ensure the penalty is not too severe
#     area_penalty = max(area_penalty, 0.5)
#
#     return yield_pred * area_penalty
...
def predict():
  ...
  #  area = float(user_df['Area (hectares)'])
   # y_user_pred_with_penalty = apply_area_penalty(area, y_user_pred_with_penalty)
